...
#(
    ...
    capture_buffer #(.DEPTH(SAMPLE_DEPTH)) u_buf (
        ...
        // Read port serves the record readout bridge in the SPI domain. Reads
        // only happen while frozen, so they never race the writer.
        .rd_clk         (spi_sclk),
        .rd_addr        (buf_rd_addr),
        .rd_en          (buf_rd_en),
        .rd_data        (buf_rd_data)
    ...
    // ---- Frozen-record readout (SPI domain) -----------------------------
    // rec_start is latched at the freeze cycle and static while frozen, so a
    // plain synchronizer is safe here (same basis as status_cdc).
